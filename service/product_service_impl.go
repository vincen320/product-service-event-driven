...
import (
	...
	"github.com/vincen320/product-service-event-driven/helper"
...
func NewProductService(repository repository.ProductRepository, validator *validator.Validate, db *mongo.Database, rdb *redis.Client, producer *nsq.Producer) ProductService {
...
func (ps *ProductServiceImpl) Create(ctx context.Context, createRequest web.ProductCreateRequest) (web.ProductResponse, error) {
	...
	//publish model to product-created-event
	ps.PublishProductCreated(helper.ToProductCreatedEvent(savedProduct))
	return response, nil
}
...
func (ps *ProductServiceImpl) PublishProductCreated(productCreateEvent event.ProductCreatedEvent) error {
	//defer ps.Producer.Stop() //TES 2x COBA INI APAKAH BISA 2X, TIDAK BISA CLOSE DI MAIN SAJA
	body, err := json.Marshal(productCreateEvent)
	if err != nil {
		return err
	}
	return ps.Producer.Publish("product-created-event", body)
	//LANJUT LINE 96
}
